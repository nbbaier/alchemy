...
> = {
  ...
  output: Out;
  /**
   * Information about the replaced resource.
   * When a resource is marked for replacement, we store its current state
   * so it can be properly cleaned up during finalization.
   */
  replacedResource?: {
    output: Out;
    props: Props;
