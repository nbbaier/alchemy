...
- `--app` - Specify which app to run in a multi-app monorepo (default: `undefined`)
- `--root-dir` - Path to the root directory of the monorepo (defaults to current directory. If `--app` is provided, then the root of your monorepo will be automatically detected.)
...
- `--env-file` - Path to environment file to load (defaults to `.env` in the CWD of your project. If `--app` is provided, then the monorepo's root `.env` will be used.)
...
# deploy the backend app (as part of a multi-app monorepo)
alchemy deploy --app backend
...
- `script` (optional): Path to the entrypoint file. Defaults to `./alchemy.run.ts` or `./alchemy.run.js`
- `--cwd` - Path to the application directory (defaults to current directory)
- `--app` - Specify which app to run in a multi-app monorepo (default: `undefined`)
- `--root-dir` - Path to the root directory of the monorepo (defaults to current directory. If `--app` is provided, then the root of your monorepo will be automatically detected.)
- `--quiet` - Suppress Create/Update/Delete messages (default: `false`) 
- `--stage` - Specify which stage/environment to target. Defaults to your username (`$USER`, or `$USERNAME` on Windows)
...
- `--force` - Apply updates to resources even if there are no changes (default: false)
- `--adopt` - Adopt existing resources that are not yet managed by your Alchemy app (default: false)
- `--tunnel` - Create a tunnel for Workers (default: false)
- `-h, --help` - Display help for command
